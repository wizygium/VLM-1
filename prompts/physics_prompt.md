...
## ZONE SYSTEM (14 zones: z0-z13)

NOTES :
#1. left and right taken from attackers view point facing the goal when defining zones
#2. distances taken from nearest goal post

| Zone | Area | Description | Depth Boundary | Lateral Boundary |
|------|------|-------------|----------------|------------------|
| z0 | Goal | Goalkeeper position, goal mouth | Inside 6m Arc | N/A |
| z1 | 6m-sideline | Far Left Wing | 6m < dist < 8.5m | LW corner between 6m and sideline out to where 9m meets sideline |
| z2 | 6m-8m | Left-Center | 6m < dist < 8m | LB interaction |
| z3 | 6m-8m | Center | 6m < dist ≤ 8m | Flat 3m segment |
| z4 | 6m-8m | Right-Center | 6m < dist < 8m | RB interaction |
| z5 | 6m-sideline | Far Right Wing | 6m < dist < 8.5m | RW corner between 6m and sideline out to where 9m meets sideline |
| z6 | 8m-10m | Far Right Back | 8m < dist < 10m | Wide RB area |
| z7 | 8m-10m | Right-Center Back| 8m < dist < 10m | Inner RB area |
| z8 | 8m-10m | Center Back | 8m < dist < 10m | Playmaker center |
| z9 | 8m-10m | Left-Center Back| 8m < dist < 10m | Inner LB area |
| z10| 8m-10m | Far Left Back | 8m < dist < 10m | Wide LB area |
...
- Center of Half-Court: (10, 20) → z12
...
3. **Team Identification (optional):**
   - Record team color if visible: main colour, or "null" if unknown
   - Based on jersey color, not position or behavior
   - there will be two referres (if visible) with shirt colours matching each other and different to the team colours and goal keeper
   - the (single) goal keeper (usually in the goal area/z0) will have colour different to attackers, defenders and referees
...
2. **All Visible Players (including goal keeper):**
